// frontend/app/admin/customers/page.jsx
"use client";
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <ExportCsvButton path="/users/customers/export.csv" label="Export Customers CSV" />
      </div>
            ...
            {customers.map((c) => (
              <tr key={c._id} className="border-b border-gray-50">
                ...
                <td className="py-3 pr-4">
                  {c.isGuest ? (
                    <span className="text-xs bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full">Guest</span>
                  ) : (
                    <span className="text-xs bg-green-50 text-green-700 px-2 py-0.5 rounded-full">Registered</span>
                  )}
                </td>
              </tr>
            ))}
            {customers.length === 0 && (
              <tr><td colSpan={5} className="py-6 text-center text-gray-400">No customers found.</td></tr>
            )}
          </tbody>
        </table>
      )}
