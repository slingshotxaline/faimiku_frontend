...
import { useListPagesQuery, useUpsertPageMutation } from "../../../features/cms/pagesApi";
...
const emptySection = () => ({ type: "richText", title: "", subtitle: "", body: "", ctaLabel: "", ctaUrl: "" });
...
export default function AdminPagesPage() {
  ...
  useEffect(() => {
    ...
    setSections(existing?.sections?.length ? existing.sections : [emptySection()]);
  }, [activeSlug, pages]);
  ...
  const removeSection = (index) => setSections((s) => s.filter((_, i) => i !== index));
    ...
    <div>
        ...
        {PAGE_PRESETS.map((p) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-sm border ${
              activeSlug === p.slug ? "border-brand-500 text-brand-500 bg-brand-50" : "border-gray-200"
            }`}
          >
            {p.title}
          </button>
      ...
      <div className="max-w-3xl space-y-6">
        <div>
          ...
          <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full border rounded px-3 py-2 text-sm mt-1" />
        </div>
        ...
        {sections.map((section, i) => (
          <div key={i} className="border border-gray-100 rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              >
                {["hero", "richText", "imageText", "faq", "cta", "custom"].map((t) => (
                  <option key={t} value={t}>{t}</option>
                ))}
              </select>
              <button onClick={() => removeSection(i)} className="text-red-500 text-sm">Remove Section</button>
            </div>
            <input
        ...
        <button onClick={addSection} className="text-sm text-brand-500 hover:underline">+ Add Section</button>
